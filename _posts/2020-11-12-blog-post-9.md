...
plot(f_MRD_raster, col = rev(colfuncYellows(100)), axes = FALSE, box = FALSE, 
     ...
     xlab = "Furnariides mean root distance", legend.width = 2)}
```
![MRD与生物多样性](images/R/Rplot48.jpeg)
#### 尺度依赖问题
...
title(main = "OLS residuals", cex = 1.5)}
```
![残差和多样性自相关](images/R/Rplot53.jpeg)
...
title(main = "Correlogram SARerr", cex = 1.5)}
```
![空间自相关的比较](images/R/Rplot54.jpeg)
那是真的没有相关。
...
plot(Bioclim.model)
```
![bioclim1](images/R/Rplot62.jpeg)
让我们绘制模型的响应变量或单个变量响应曲线。
```r
response(Bioclim.model)
```
![bioclim2](images/R/Rplot63.jpeg)
进行分布区预测
```r
...
```
![bioclim3](images/R/Rplot65.jpeg)
测试数据
```r
...
```
如随机森林的变量响应模型：
![rf_eval_strip](images/R/Rplot88.jpeg)
```r
#运行集成模型
...
```
![dis](images/R/Rplot91.png)
![modelcom](images/R/Rplot92.png)
### 多变量模型拟合与预测
```r
## setup environment ----
setwd('workdir')
...
```
![kissmig1](images/R/Rplot93.jpeg)
```r
# 运行kissmig并输出各种东西
...
```
![kissmig2](images/R/Rplot94.jpeg)
```r
# 探索地理空间
