...
```
![barplot](images/Rmodel/Rplot12.jpeg)
#### 准备空间数据
准备运行环境
```r
library(marmap)
...
```
![bathydata](images/Rmodel/Rplot13.jpeg)
...
```
![path](images/Rmodel/Rplot14.jpeg)
计算最小耗费距离矩阵
```r
...
```
![SSCA](images/Rmodel/Rplot20.png)
组合图片
```r
# 组合两张关于温度的
...
```
![cor](images/Rmodel/Rplot22.jpeg)
识别重要变量
```r
# 使用前向选择来确定重要的环境变量
...
screeplot(rda1)
```
![rda1](images/Rmodel/Rplot23.jpeg)
### RDA的可视化
```r
# 创建一个数据框来正确地为区域着色
...
dev.off()
```
![rda2](images/Rmodel/Rplot24.png)
### 部分冗余分析
...
dev.off()
```
![rda3](images/Rmodel/Rplot25.png)
...
```
![rda4](images/Rmodel/Rplot26.jpeg)
确定分布尾部的SNP
...
```
![vpa](images/Rmodel/Rplot27.jpeg)
### 全流程方差分解
来自微信公众号：生态R学社
加载数据
```r
...
title('VPA')
```
![vpa](images/Rmodel/Rplot28.jpeg)
接下来需要检验方差结果的显著性。
```r
...
![vpa](images/Rmodel/Rplot29.jpeg)