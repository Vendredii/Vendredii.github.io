...
```
可得：
![ggplot导出的简单散点图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot4.jpeg)
...
ggplot(morph, aes(sex, wingl)) +
  geom_violin()
```
![简单小提琴图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot5.jpeg)
```
#r通过箱型图翅膀长度和物种间的关系，箱型图中的小点为异常值
ggplot(morph, aes(taxon, wingl)) + geom_boxplot()
```
![简单箱型图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot6.jpeg)
结果发现下面的物种名字我们都看不见了……
因此我们使用`coord()`来设置坐标系，就非常好了
```r
ggplot(morph, aes(taxon, wingl)) + geom_boxplot() + coord_flip()
```
![改良箱型图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot7.jpeg)
...
```
![分类上色图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot8.jpeg)
我们还可以通过图例里再加一个别的数据，如上喙长(ubeakl)，实现表格数据的套娃：
```r
ggplot(morph, aes(wingl, beakh)) +
  geom_point(aes(size = ubeakl))
```
![增加了一个数据](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot9.jpeg)
这样可能太丑了，我们需要换一种方式来表达：
```r
ggplot(morph, aes(wingl, beakh)) +
geom_point(aes(colour = ubeakl))
```
![改良后的上图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot10.jpeg)
那我们可以把这些参数都加入，制作一张终极套娃：
![改良后的上图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot11.jpeg)
...
```
![分类后图](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot12.jpeg)
或：
```r
ggplot(morph, aes(wingl, beakh)) + geom_point() +
  facet_grid(sex~taxon)
```
![分类后图2](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot13.jpeg)
...
ggplot(morph_quant, aes(x = taxon, y = m, ymin = l, ymax = u)) +
  ...
  coord_flip()
```
![分类后图2](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot14.jpeg)
...
dev.off()
```
![系统发育树](http://github.com/Vendredii/Vendredii.github.io/blob/master/images/R/Rplot95.jpg)